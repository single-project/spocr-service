...
import org.century.scp.spocr.legaltype.services.LegalTypeServiceFacade;
...
public class LegalTypeController {
  ...
  private final LegalTypeServiceFacade legalTypeService;

  @GetMapping("/{id}")
  public ResponseEntity<LegalTypeView> getItem(@PathVariable(value = "id") long id) {
    return ResponseEntity.ok(legalTypeService.get(id));
  }
  ...
  public ResponseEntity<Page<LegalTypeView>> getItems(
      ...
      Pageable pageable) {
    return ResponseEntity.ok(legalTypeService.get(legalTypeSpecification, pageable));
  }
}
